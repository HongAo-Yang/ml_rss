...
def VASP_generate_POSCAR_and_KPOINTS(input_dir_name,
                                     input_file_name,
                                     output_dir_name):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    atoms_group = None
    if rank == 0:
        print("[log] Running VASP_generate_POSCAR_and_KPOINTS")
        if not os.path.isdir(input_dir_name):
            raise RuntimeError('input dir %s does not exist' %
                               (input_dir_name))
        if not os.path.isdir(output_dir_name):
    ...
    for atom in atoms_local:
        ...
        with open(os.path.join(config_dir_name, "ASE_VASP_ORDER"), "w") as forder:
            forder.writelines([str(x) + "\n" for x in order])
        os.system("cp {}/* {}".format(input_dir_name, config_dir_name))
        with open(os.path.join(config_dir_name, "KPOINTS"), 'w') as f:
            f.write('k-points generated by VASP_generate_POSCAR_and_KPOINTS\n0\nG\n')
            f.write('%d %d %d\n' % (k1, k2, k3))
            f.write('0 0 0\n')
    if rank == 0:
        print("[log] Finished VASP_generate_POSCAR_and_KPOINTS")
